"""Automatic JSON Schema extraction from Python types.
...
Supports:
  ...
  - ``@schema`` decorator for adding metadata

The public entry point is :func:`extract_schema`, which returns a
JSON-Schema-compatible ``dict`` for any supported type.
"""
...
class FieldMeta:
    """Attach JSON Schema metadata to a field via ``Annotated[str, FieldMeta(description=...)]``."""
...
def extract_schema(cls: type) -> dict[str, Any]:
    ...
    Raises :class:`TypeError` if *cls* is not a supported type.
    """
...
def _schema_from_annotations(cls: type) -> dict[str, Any]:
    ...
    if hasattr(cls, "__schema_description__"):
        schema["description"] = cls.__schema_description__
    if hasattr(cls, "__schema_title__"):
        schema["title"] = cls.__schema_title__
    elif hasattr(cls, "__name__"):
        schema["title"] = cls.__name__
    if hasattr(cls, "__schema_examples__"):
        schema["examples"] = cls.__schema_examples__
...
def _type_to_schema(tp: Any) -> tuple[dict[str, Any], FieldMeta | None]:
    ...
    # --- Enum subclasses ---
    if inspect.isclass(tp) and issubclass(tp, __import__("enum").Enum):
...
def _is_annotated(tp: Any) -> bool:
    ...
    # Python 3.11+ has typing.get_origin returning Annotated
    try:
        ...
        pass
    try:
        from typing_extensions import Annotated, get_origin as ext_get_origin
